...
?>
				...
				<?php } ?>
                <?php if($row1['sendToExec'] == 0) { ?>
                    <a href="sendToExe.php?policy_id=<?php echo $row1['policy_id'] ?>" class="btn btn-sm btn-primary pull-right">ارسال الى المدير التنفيذي</a>  
                <?php } else { ?>
                    <span class="label label-info pull-right"><i class="fa fa-check"></i> تم الارسال الى المدير التنفيذي</span>
                <?php } ?>
				<br>
			</div>
...
if(isset($_POST['addHeadReview'])) {
	...
	$policy_group_id = $_POST['policy_group_id'];
	 ...
	 $sql1 = "UPDATE policy SET headReview='$review' WHERE policy_id=$policy_id and group_id=$policy_group_id";
